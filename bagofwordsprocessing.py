...
def tokenize_corpus(path, train=True):
  ...
  for w in stopWords:
      if((w == "not") or (w == "no") or (w == "t") or (w == "nor") or (w == "should") or (w == "shouldn") or (w == "would") or (w == "wouldn") or (w == "must") or (w == "mustn") or ((w == "need") or (w == "needn"))):
          stopWords.remove(w)
  classes = []
  samples = []
  docs = []
...
def main(argv):
  ...
  traintxt = path+"/essay9_sample.txt"
  ...
  #with open(path+"/"+outputfTest+"_bag_of_words_"+str(word_count_threshold)+".csv", "w") as f:
    #writer = csv.writer(f)
  ...
  #outfile = open(path+"/"+outputfTest+"_classes_"+str(word_count_threshold)+".txt", 'w')
  #outfile.write("\n".join(classesTest))
  #outfile.close()
  ...
  #outfile = open(path+"/"+outputfTest+"_samples_class_"+str(word_count_threshold)+".txt", 'w')
  #outfile.write("\n".join(samplesTest))
  #outfile.close()
